...
func main() {
	...
	r.GET("/music", func(c *gin.Context) {
		...
		result := db.Order("id desc").Limit(5).Find(&music)
		if result.Error != nil {
			c.JSON(http.StatusInternalServerError, gin.H{
				"message": "query error.",
			})
		...
		if m.Picture() != nil {
			...
			picdata = base64.StdEncoding.EncodeToString(m.Picture().Data)
		} else {
			picext = "image/png"
